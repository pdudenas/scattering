...
class Reduction:
    ...
    def __init__(self, name, SDD, center_x, center_y, xpixels = 1475, ypixels = 1679,
                 pixel_size = .172,wavelength = .123894,split_idx=2):
        self.name = name
        ...
        split2 = split1[split_idx].split('p') # use 2nd appended alpha, as that is read from the hardware
        self.alphai = float(split2[1])/1000.0

    def raw_plot(self,size=(6,4.5)):
        fig, ax = plt.subplots(figsize=size)
        plt.imshow(np.log(self.data),cmap='jet')
        plt.colorbar()
        plt.show()

    def plot(self,size=(6,4.5),show_cbar='False'):
        fig, ax = plt.subplots(figsize=size)
...
if __name__ == '__main__':
    ...
    os.chdir('/data733-data/bl733data-2017/userdata/weber/2019_04_12/PSPAA')
#    import time
    ...
    image1 = Reduction('GNPa_A0p180_2m.edf',1990, 622, 1320)
    print(image1.alphai)
    data = image1.data
    image1.plot()
